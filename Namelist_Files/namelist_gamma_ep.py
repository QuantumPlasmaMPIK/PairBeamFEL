...
RNG_initial_seed = int(120 * datetime.datetime.today().month + 31 * datetime.datetime.today().day) # + 24 * datetime.datetime.today().hour)
...
class BeamSpecies:
    def __init__(self, species_name):
        ...
        self.beam_longitudinal_length   = 0                     # longitudinal full-length (for initial bunch)
        self.beam_transverse_length = 0                         # transverse half-length (for initial bunch)
        self.slice_longitudinal_length   = 0                    # longitudinal full-length (per-slice)
        self.slice_transverse_length = 0                        # transverse half-length (per-slice)
...
simulation_relativistic_Poisson_initialization   = False #True if simulation_3d else False    # in 3d, optimization via relativistic Poisson initializer is important
simulation_Poisson_initialization = False #False if simulation_relativistic_Poisson_initialization else True   # in 2d, proper field decay is more critical than the usage of relativistic Poisson
...
bunch_energy = 48000.0                                                  # initial bunch energy in MeV
bunch_gamma = bunch_energy / electron_mass_MeV                       # initial average lorentz factor for the beam
Q_electron_bunch   = -1250 * 1 * pC                                   # Total charge of the electron bunch
beam_longitudinal_sigma = 2.5                                       # Beam longitudinal spatial sigma in um.
beam_transverse_sigma = 6.                                          # Beam transverse/radial rms size (cylindrical symmetry) in um.
number_of_slices = 500                                                 # number of internal microbunches (slices) within the beam
simulated_slices = 3                                    # number of adjecent slices that will be simulated
bunching_factor = 0.7
slice_interval = 6e-3   # in um, should be consistent with number of slices & initial bunch size
Q_electron_bunch /= number_of_slices
slice_sigma = beam_longitudinal_sigma / number_of_slices
slice_FWHM = slice_sigma * 2.35
if bunching_factor > 0:
    slice_FWHM = 2e-3    # should be manually calculated before!
    slice_sigma = slice_FWHM / 2.35
slice_FWHM_cut = 1.5   # as default, 2 --> total bunch size = cut * FWHM

bunch_energy_spread = 3e-4                                           # initial rms energy spread / average energy (not in percent)
bunch_normalized_emittance = 1e-1                                     # initial beam normalized emittance (beam divergence X transverse beam size), same for both transverse planes (in um_rad) --> valid for electron & positron
...
beam_species = ["electron", "positron"]                                          # name of the species contained by the beam
...
species_macroparticle_number = int(6e6)                              # number of macroparticles to be used to define a species within the beam
num_of_undulator_periods = 800                                       # number of periodic arrangements of dipole magnets with alternating polarity
...
box_longutidunal_beam_factor = 8                                     # simulation_box_x / beam_full_length_x
...
diagnostics_frequency = 75                                           # Diagnostics I/O frequency
simulation_duration_fraction = 1                                  # default is 1
...
bunch_density = -Q_electron_bunch * 1e12 / (math.pi * (2 * beam_transverse_sigma) ** 2 * slice_FWHM)   # in cm^-3
...
if "electron" in BeamSpeciesDictionary:
    ...
    BeamSpeciesDictionary["electron"].slice_longitudinal_length  = slice_FWHM_cut * slice_FWHM  # rather than slice_FWHM_cut
    BeamSpeciesDictionary["electron"].slice_transverse_length    = 3 * beam_transverse_sigma
...
if "positron" in BeamSpeciesDictionary:
    ...
    BeamSpeciesDictionary["positron"].slice_longitudinal_length  = slice_FWHM_cut * slice_FWHM # rather than slice_FWHM_cut
    BeamSpeciesDictionary["positron"].slice_transverse_length    = 3 * beam_transverse_sigma
...
if "proton" in BeamSpeciesDictionary:
    ...
    BeamSpeciesDictionary["proton"].slice_longitudinal_length  = slice_FWHM_cut * slice_FWHM # rather than slice_FWHM_cut
    BeamSpeciesDictionary["proton"].slice_transverse_length    = 3 * beam_transverse_sigma
...
def norm_ppf(p):
    ...
    q = p - 0.5
    r_small = (p < 0.02425)
    r_mid   = (~r_small) & (p <= 1 - 0.02425)
    r_large = (p > 1 - 0.02425)
    x = np.empty_like(p, dtype=float)
    if np.any(r_small):
        r = np.sqrt(-2.0*np.log(p[r_small]))
        x[r_small] = (((((c[0]*r + c[1])*r + c[2])*r + c[3])*r + c[4])*r + c[5]) / \
                      ((((d[0]*r + d[1])*r + d[2])*r + d[3])*r + 1.0)
    if np.any(r_mid):
        r = q[r_mid]**2
        x[r_mid] = (((((a[0]*r + a[1])*r + a[2])*r + a[3])*r + a[4])*r + a[5]) * q[r_mid] / \
                    (((((b[0]*r + b[1])*r + b[2])*r + b[3])*r + b[4])*r + 1.0)
    if np.any(r_large):
        r = np.sqrt(-2.0*np.log(1.0 - p[r_large]))
        x[r_large] = -(((((c[0]*r + c[1])*r + c[2])*r + c[3])*r + c[4])*r + c[5]) / \
                        ((((d[0]*r + d[1])*r + d[2])*r + d[3])*r + 1.0)
    return x

# Truncated normal sampling for longitudinal coordinate
def truncated_normal(n, mean, sigma, lower, upper, rng):
    a = (lower - mean) / sigma
    b = (upper - mean) / sigma
    # Standard normal CDF for scalars a,b using math.erf
    Phi_a = 0.5*(1.0 + math.erf(a/math.sqrt(2.0)))
    Phi_b = 0.5*(1.0 + math.erf(b/math.sqrt(2.0)))
    u = rng.uniform(Phi_a, Phi_b, size=n)
    z = norm_ppf(u)
    return mean + sigma * z

# 2d test scatter visualization
def test_vis(x, y, fig_name):
    fig, ax = plt.subplots()
    ax.scatter(x, y, s=0.0001, alpha=0.8)
    plt.tight_layout()
    plt.savefig(fig_name, format="png", pad_inches=0.02, dpi=140)
    plt.close(fig)
...
for species_type in BeamSpeciesDictionary:
    ...
    species.bunch_position_array       = np.zeros((position_array_size, int(npart * simulated_slices) ))                         # positions x, y, (z), weight
    species.bunch_momentum_array       = np.zeros((3, int(npart * simulated_slices)))                                           # momenta x, y, z
    for slice_index in  range(0, simulated_slices):
        # longitudinal & transverse sampled slice coordinates
        ...
        RNG_initial_seed += 1
        position_x = truncated_normal(npart, mean=0, sigma=slice_sigma,
                                      lower=-1 * slice_FWHM_cut * slice_FWHM / 2, upper=1 * slice_FWHM_cut * slice_FWHM / 2, rng=RNG)
        position_x += slice_index * slice_interval   # shift for the adjecent slice
        RNG = np.random.default_rng(RNG_initial_seed)  # single sampling is done for the intial mirror, the rest will follow the phase-shifting
        RNG_initial_seed += 1
        position_y = truncated_normal(npart, mean=0, sigma=beam_transverse_sigma,
                                      lower=-3 * beam_transverse_sigma, upper=3 * beam_transverse_sigma, rng=RNG)
        RNG = np.random.default_rng(RNG_initial_seed)  # single sampling is done for the intial mirror, the rest will follow the phase-shifting
        RNG_initial_seed += 1
        position_z = truncated_normal(npart, mean=0, sigma=beam_transverse_sigma,
                                      lower=-3 * beam_transverse_sigma, upper=3 * beam_transverse_sigma, rng=RNG)
        ...
        ### Q_total is the charge per slice there!
        ...
        Q_part                             = species.Q_total / npart                                        # charge per macro-particle in the slice
        ...
        species.bunch_position_array[0, int(slice_index * npart):int((slice_index+1) * npart)] = position_x
        species.bunch_position_array[1, int(slice_index * npart):int((slice_index+1) * npart)] = position_y
        if simulation_3d:
            species.bunch_position_array[2, int(slice_index * npart):int((slice_index+1) * npart)] = position_z
            species.bunch_position_array[3, int(slice_index * npart):int((slice_index+1) * npart)] = np.multiply(np.ones(npart), species.weight)
        else:
            species.bunch_position_array[2, int(slice_index * npart):int((slice_index+1) * npart)] = np.multiply(np.ones(npart), species.weight)

        RNG = np.random.default_rng(RNG_initial_seed)  # single sampling is done for the intial mirror, the rest will follow the phase-shifting
        RNG_initial_seed += 1
        species.bunch_momentum_array[0, int(slice_index * npart):int((slice_index+1) * npart)] = RNG.normal(loc=species.mass * species.v0 * species.gamma, scale= species.mass * species.v0 * species.gamma * species.bunch_energy_spread, size=npart)      # p_x --> generate normal distr. with mean of relativistic momentum (in mc) and std of energy spread
        RNG = np.random.default_rng(RNG_initial_seed)  # single sampling is done for the intial mirror, the rest will follow the phase-shifting
        RNG_initial_seed += 1
        species.bunch_momentum_array[1, int(slice_index * npart):int((slice_index+1) * npart)] = RNG.normal(loc=0, scale=species.mass * species.bunch_normalized_emittance / beam_transverse_sigma, size=npart)       # p_y -> generate normal distr. with mean of 0 and std of beam divergence (in rad)
        RNG = np.random.default_rng(RNG_initial_seed)  # single sampling is done for the intial mirror, the rest will follow the phase-shifting
        RNG_initial_seed += 1
        species.bunch_momentum_array[2, int(slice_index * npart):int((slice_index+1) * npart)] = RNG.normal(loc=0, scale=species.mass * species.bunch_normalized_emittance / beam_transverse_sigma, size=npart)       # p_z --> generate normal distr. with mean of 0 and std of beam divergence (in rad)
        if not momentum_z_include:
            species.bunch_momentum_array[2, int(slice_index * npart):int((slice_index+1) * npart)] = 0

    # Vis.
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[1,:] / um, "labframe_x_y_" + species.species_name + ".png")
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[2,:] / um, "labframe_x_z_" + species.species_name + ".png")
    test_vis(species.bunch_momentum_array[0,:], species.bunch_momentum_array[1,:], "labframe_px_py_" + species.species_name + ".png")
    ...
    bunch_time_in_lab_frame = np.zeros(int(npart * simulated_slices))                  # initial time is set to zero in lab frame for all particles
    ...
    U_t = np.ones(int(npart * simulated_slices)) * bunch_gamma_factors                         # bunch U_0 in lab-frame
    ...
    # Vis.
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[1,:] / um, "bunchrestframe_x_y_" + species.species_name + ".png")
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[2,:] / um, "bunchrestframe_x_z_" + species.species_name + ".png")
    test_vis(species.bunch_momentum_array[0,:], species.bunch_momentum_array[1,:], "bunchrestframe_px_py_" + species.species_name + ".png")
...
labframe_bunch_size = BeamSpeciesDictionary["electron"].slice_longitudinal_length #+ (simulated_slices - 1) * slice_interval               # already given in lab frame
...
reference_bunch_size = labframe_bunch_size * BeamSpeciesDictionary["electron"].longitudinal_spatial_correction   # electron beam is considered as reference
...
Ly = box_transverse_beam_factor * BeamSpeciesDictionary["electron"].slice_transverse_length         # Dimension of the box in y
...
Tsim = simulation_duration_fraction * required_simulation_time + 0.4 * Lx                # simulation time


## apply bunch shift to centralize it (currently common for all species)
for species_type in BeamSpeciesDictionary:
    species = BeamSpeciesDictionary[species_type]
    species.bunch_position_array[0, :] += Lx * 0.35 - np.mean(species.bunch_position_array[0, :]) # shift bunch center to the box center
    ...
    # Vis.
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[1,:] / um, "boxcentered_x_y_" + species.species_name + ".png")
    test_vis(species.bunch_position_array[0,:] / um, species.bunch_position_array[2,:] / um, "boxcentered_x_z_" + species.species_name + ".png")
...
Species(
  name = "electron",
  position_initialization = BeamSpeciesDictionary["electron"].bunch_position_array[:, int(2*npart):int(3*npart)],
  momentum_initialization = BeamSpeciesDictionary["electron"].bunch_momentum_array[:, int(2*npart):int(3*npart)],
...
Species(
  name = "electron_1",
  time_frozen = 0,
  position_initialization = BeamSpeciesDictionary["electron"].bunch_position_array[:, int(npart):int(2*npart)],
  momentum_initialization = BeamSpeciesDictionary["electron"].bunch_momentum_array[:, int(npart):int(2*npart)],
  #momentum_initialization = "maxwell-juettner",
  #mean_velocity          = [beam_longitudinal_average_velocity, 0, 0],
  #temperature             = [0.00001, 0.00001, 0.],
  mass = 1.0,
...
Species(
  name = "electron_2",
  time_frozen = 0,
  position_initialization = BeamSpeciesDictionary["electron"].bunch_position_array[:, int(0):int(npart)],
  momentum_initialization = BeamSpeciesDictionary["electron"].bunch_momentum_array[:, int(0):int(npart)],
  #momentum_initialization = "maxwell-juettner",
  #mean_velocity          = [beam_longitudinal_average_velocity, 0, 0],
  #temperature             = [0.00001, 0.00001, 0.],
  mass = 1.0,
...
if "positron" in BeamSpeciesDictionary:
    Species(
      name = "positron",
      position_initialization = BeamSpeciesDictionary["positron"].bunch_position_array[:, int(2*npart):int(3*npart)],
      momentum_initialization = BeamSpeciesDictionary["positron"].bunch_momentum_array[:, int(2*npart):int(3*npart)],
      #momentum_initialization = "maxwell-juettner",
      #mean_velocity          = [beam_longitudinal_average_velocity, 0, 0],
      #temperature             = [0.00001, 0.00001, 0.],
      mass = 1.0,
    ...
    Species(
      name = "positron_1",
      time_frozen = 0,
      position_initialization = BeamSpeciesDictionary["positron"].bunch_position_array[:, int(npart):int(2*npart)],
      momentum_initialization = BeamSpeciesDictionary["positron"].bunch_momentum_array[:, int(npart):int(2*npart)],
      #momentum_initialization = "maxwell-juettner",
      #mean_velocity          = [beam_longitudinal_average_velocity, 0, 0],
      #temperature             = [0.00001, 0.00001, 0.],
      mass = 1.0,
    ...
    Species(
      name = "positron_2",
      time_frozen = 0,
      position_initialization = BeamSpeciesDictionary["positron"].bunch_position_array[:, int(0):int(npart)],
      momentum_initialization = BeamSpeciesDictionary["positron"].bunch_momentum_array[:, int(0):int(npart)],
      #momentum_initialization = "maxwell-juettner",
      #mean_velocity          = [beam_longitudinal_average_velocity, 0, 0],
      #temperature             = [0.00001, 0.00001, 0.],
      mass = 1.0,
...
transverse_field_plane_front_index = int(np.searchsorted( np.linspace(0, Lx, int(Lx/dx + 1), dtype = "float64"), Lx/2. + 2. * reference_bunch_size, 'left'))
...
# for the transverse screen (y-z plane, with some thickness) in front of the beam center
transverse_field_plane_front_index_2 = int(np.searchsorted( np.linspace(0, Lx, int(Lx/dx + 1), dtype = "float64"), 1050*um, 'left'))
logging.info("FEL transverse field diagnostics (for 2nd slice) screen position is assigned to (in um):  {:.5e} ".format(transverse_field_plane_front_index_2 * dx / um))
DiagFields(
    every = int(Tsim / dt / (diagnostics_frequency) ),
    #flush_every           = int(5 * Tsim / dt / diagnostics_frequency ),
    fields = ["Ey", "Ez", "By_m", "Bz_m", "Jx_electron", "Rho_electron"],
    subgrid = np.s_[transverse_field_plane_front_index_2, :, :] if simulation_3d else np.s_[transverse_field_plane_front_index_2, :]  # 2d screen (or 1d line) (reduction is required for 3d geometry)
)
...
def my_filter_random_select(particles) :
    ...
    prob = 0.15                            # 20% probability for chosen particles
...
for species_type in BeamSpeciesDictionary:
    ...
    for slice_index in range(1,simulated_slices):
        DiagTrackParticles(
                species               = species_type + "_" + str(slice_index),
                every                 = int(Tsim / dt / diagnostics_frequency),
                #flush_every           = int(5 * Tsim / dt / diagnostics_frequency ),
                attributes            = tracking_attributes,
                filter=my_filter_random_select
