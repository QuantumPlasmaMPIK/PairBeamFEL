...
undulator_period = 31000                                             # laser radiation wavelength in um
...
bunch_energy = 350                                                  # initial bunch energy in MeV
bunch_gamma = bunch_energy / electron_mass_MeV                       # initial average lorentz factor for the beam
Q_electron_bunch   = -200 * 1 * pC                                   # Total charge of the electron bunch
longitudinal_rms = 4 * 10    # in um
beam_longitudinal_sigma = math.sqrt(2 * math.pi) *  longitudinal_rms / 2       # Beam longitudinal spatial sigma in um.
beam_transverse_sigma = 200 / math.sqrt(10)                                       # Beam transverse/radial rms size (cylindrical symmetry) in um.
beam_fraction = 0.5                                                 # normalized fraction of the beam that will be simulated, default is 1, i.e. full beam (useful in Xray sim.)
...
num_of_undulator_periods = 350                                       # number of periodic arrangements of dipole magnets with alternating polarity
...
box_longutidunal_beam_factor = 3                                     # simulation_box_x / beam_full_length_x
...
Tsim = 1 * required_simulation_time                 # simulation time
...
transverse_field_plane_front_index = int(np.searchsorted( np.linspace(0, Lx, int(Lx/dx + 1), dtype = "float64"), Lx/2. + 0.7 * reference_bunch_size, 'left'))
