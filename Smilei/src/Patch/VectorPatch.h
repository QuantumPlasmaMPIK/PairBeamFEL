...
// ---------------------------------------------------------------------------------------------------------------------
//! This structure contains the properties of Radiation Detector
// ---------------------------------------------------------------------------------------------------------------------
struct RadiationDetector {
	//! Pixel Section where the info will be distributed over the pixels, handled independently
	std::vector<double> pixel_positions_x;
	std::vector<double> pixel_positions_y;
	std::vector<double> pixel_positions_z;
	std::vector<bool>   pixel_integration_exceeded;  // initially false, as there is no deposition at the beginning
	std::vector<std::vector<double>> pixel_electric_field_x; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
	std::vector<std::vector<double>> pixel_electric_field_y; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
	std::vector<std::vector<double>> pixel_electric_field_z; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
	std::vector<std::vector<double>> pixel_magnetic_field_x; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
	std::vector<std::vector<double>> pixel_magnetic_field_y; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
	std::vector<std::vector<double>> pixel_magnetic_field_z; // each internal vector will correspond to the deposited field of a given pixel id, and field size = time array size
...
public :
    ...
    //! Radiation Detector
	RadiationDetector Radiation_Detector; // the main detector whose values will be dumped to the diagnostics file at the end of the simulation (should be used in restart case too!)
	std::vector<RadiationDetector> ThreadBasedRadiationDetectors; // patch-specific detectors, just to provide private work-space for each patch to avoid race condition
...
#  ifdef _PARTEVENTTRACING  
	...
	RadiationDetector getEmptyRadiationDetector(Params &params);

    //! initialize the Radiation Detector via corresponding namelist parameters
    void initRadiationDetector(Params &params, SmileiMPI *smpi);

    //! field deposition for the Radiation Detector
    void depositFields(Params &params, SmileiMPI *smpi, double retarded_simulation_time, double timestep);

    //! write deposited fields to the file at the end of simulation
    void dumpRadiationDetector( Params &params, SmileiMPI *smpi);
