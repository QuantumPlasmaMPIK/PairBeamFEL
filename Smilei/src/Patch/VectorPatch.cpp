...
//! initialize the Radiation Detector with dummy (zero) field values
RadiationDetector VectorPatch::getEmptyRadiationDetector(Params &params)
{
	RadiationDetector EmptyDetector;
	//! Pixel Section where the info will be distributed over the pixels, handled independently
	std::copy(params.pixel_positions_x.begin(), params.pixel_positions_x.end(), std::back_inserter(EmptyDetector.pixel_positions_x));
	std::copy(params.pixel_positions_y.begin(), params.pixel_positions_y.end(), std::back_inserter(EmptyDetector.pixel_positions_y));
	std::copy(params.pixel_positions_z.begin(), params.pixel_positions_z.end(), std::back_inserter(EmptyDetector.pixel_positions_z));
	EmptyDetector.pixel_integration_exceeded = std::vector<bool>(EmptyDetector.pixel_positions_x.size(), false); // pixels' integrations initially not exceeded
	EmptyDetector.pixel_number = EmptyDetector.pixel_positions_x.size();
	//! Detector Section whose info is common for all pixels
	std::copy(params.detector_time_array.begin(), params.detector_time_array.end(), std::back_inserter(EmptyDetector.time_array));
	if (EmptyDetector.time_array.size() > 1){
		EmptyDetector.time_resolution = EmptyDetector.time_array[1] - EmptyDetector.time_array[0];
	}
	//! Electric Field initialization for each pixel independently, initially the field values are zero, means no deposition yet.
	for( unsigned int pixel_id=0; pixel_id < EmptyDetector.pixel_number; pixel_id++ ) {
		EmptyDetector.pixel_electric_field_x.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0)); // field count = interval count = # of time index - 1
		EmptyDetector.pixel_electric_field_y.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0));
		EmptyDetector.pixel_electric_field_z.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0));
		EmptyDetector.pixel_magnetic_field_x.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0)); // field count = interval count = # of time index - 1
		EmptyDetector.pixel_magnetic_field_y.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0));
		EmptyDetector.pixel_magnetic_field_z.push_back(std::vector<double>(EmptyDetector.time_array.size()-1, 0));
	}
	return EmptyDetector;
}

//! initialize the Radiation Detector via corresponding namelist parameters
void VectorPatch::initRadiationDetector(Params &params, SmileiMPI *smpi)
{
	//! initialize the base detector
	Radiation_Detector = getEmptyRadiationDetector(params);
	unsigned int number_of_threads_ = smpi->getOMPMaxThreads();
	//! initialize the sub detectors
	for( unsigned int ipatch=0 ; ipatch < number_of_threads_ ; ipatch++ ) {
		ThreadBasedRadiationDetectors.push_back(getEmptyRadiationDetector(params));
	}
	MESSAGE( "\n\tRadiation Detector is initialized with pixel number and time-grid size: " << Radiation_Detector.pixel_number << " - " << Radiation_Detector.time_array.size());
	if (Radiation_Detector.time_array.size() > 0)
		MESSAGE( "\n\tDetector time-array boundaries: " << Radiation_Detector.time_array[0] << " - " << Radiation_Detector.time_array.back() );

	// Check the INITIALIZATION
	/*
	std::cout << smpi->getRank() << std::endl;
	std::cout << "Check detector electric field initialization\n";
	std::cout << "Detector time array size: " << Radiation_Detector.time_array.size() << std::endl;
	for( unsigned int pixel_id=0; pixel_id < Radiation_Detector.pixel_number; pixel_id++ ) {
		std::cout << "pixel: " << pixel_id << " --> " << Radiation_Detector.pixel_positions_x[pixel_id] << " , "  << Radiation_Detector.pixel_positions_y[pixel_id] << " , "  << Radiation_Detector.pixel_positions_z[pixel_id] << std::endl;
		std::cout << std::boolalpha << "integration exceeded for a given pixel: " << Radiation_Detector.pixel_integration_exceeded[pixel_id] << std::endl;
		unsigned int field_grid_size = Radiation_Detector.pixel_electric_field_x[pixel_id].size();
		std::cout << "Grid size of the electric field in the pixel: " << field_grid_size  << std::endl;
		for( unsigned int t_id=0; t_id < field_grid_size; t_id++ ) {
 			std::cout << Radiation_Detector.pixel_electric_field_x[pixel_id][t_id] << " - " << Radiation_Detector.pixel_electric_field_y[pixel_id][t_id] << " - " << Radiation_Detector.pixel_electric_field_z[pixel_id][t_id] << std::endl;
		}
 		std::cout << std::endl;
	}
	*/
	...
	//! Resize & initialize particle-related algorithm parameters (detector pixel size & recent deposition time) are already handled in Patch & Species create - initialization functions.
}


//! field deposition for the Radiation Detector
void VectorPatch::depositFields(Params &params, SmileiMPI *smpi, double retarded_simulation_time, double timestep)
{
	// thread-based parameters definitions
	double particle_velocity[3], particle_velocity_old[3], particle_acceleration[3]; // particle array properties
	double mass, gf, gf_old, weight_charge_factor; // particle scalar properties
	double R[3], n[3]; // distance and normalized distance vectors
	double distance, detector_time, field_denominator, scale_factor; // deposited field-related calculations
	double temp_holder[3], cross_product_holder[3]; // deposited field-related calculations
	double deposited_electric_field[3], deposited_magnetic_field[3]; // deposited field-related calculations
	double recent_deposition_time, recent_deposited_field_x, recent_deposited_field_y, recent_deposited_field_z; // particle-deposited field related helper variables
	double recent_deposited_Bfield_x, recent_deposited_Bfield_y, recent_deposited_Bfield_z; // particle-deposited field related helper variables
	unsigned int deposition_insertion_index, previous_deposition_index; // deposited field-related calculations

	unsigned int particle_spatial_dimension = patches_[0]->vecSpecies[0]->nDim_particle; // 2 for 2d, and 3 for 3d


	#pragma omp for schedule(dynamic) // Each ptch will have different number of particles, so dynamic scheduling applied rather than static
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {  // PATCH-BASED LOOP (Where Smilei parallelism takes place)
    	unsigned int workspace_id = omp_get_thread_num();
    	for( unsigned int pixel_id=0 ; pixel_id < Radiation_Detector.pixel_number ; pixel_id++ ){ // DETECTOR PIXEL-BASED LOOP
    		std::vector<double>& DetectorPixelFieldX = ThreadBasedRadiationDetectors[workspace_id].pixel_electric_field_x[pixel_id]; // helper l-values to the fields of specific pixel
    		std::vector<double>& DetectorPixelFieldY = ThreadBasedRadiationDetectors[workspace_id].pixel_electric_field_y[pixel_id];
    		std::vector<double>& DetectorPixelFieldZ = ThreadBasedRadiationDetectors[workspace_id].pixel_electric_field_z[pixel_id];
    		std::vector<double>& DetectorPixelBFieldX = ThreadBasedRadiationDetectors[workspace_id].pixel_magnetic_field_x[pixel_id]; // helper l-values to the fields of specific pixel
			std::vector<double>& DetectorPixelBFieldY = ThreadBasedRadiationDetectors[workspace_id].pixel_magnetic_field_y[pixel_id];
			std::vector<double>& DetectorPixelBFieldZ = ThreadBasedRadiationDetectors[workspace_id].pixel_magnetic_field_z[pixel_id];
    		for (const auto & species : patches_[ipatch]->vecSpecies) { // SPECIES LOOP
    			unsigned int number_of_particles = species->particles->size();
    			for( unsigned int particle_id=0 ; particle_id < number_of_particles ; particle_id++ ){ // PARTICLES LOOP (Main loop as RaDiO algorithm is particle-based)
    				if (species->particles->RecentDepositionTimes[pixel_id][particle_id] == 0){ // pre-check to avoid possible init. problems, should be assigned to detector start time
    					species->particles->RecentDepositionTimes[pixel_id][particle_id] = Radiation_Detector.time_array[0];
    				}
    				if (species->particles->RecentDepositionTimes[pixel_id][particle_id] < Radiation_Detector.time_array.back()){ // check whether pixel deposition is exceeded or not for a given particle
    					// get normalized distance and detection time
						R[0] = Radiation_Detector.pixel_positions_x[pixel_id] - species->particles->Position[0][particle_id];
						R[1] = Radiation_Detector.pixel_positions_y[pixel_id] - species->particles->Position[1][particle_id];
						R[2] = Radiation_Detector.pixel_positions_z[pixel_id]; // 2-dim. case --> particle motion plane is directly on the x-y plane
						if (particle_spatial_dimension == 3) // check 3-dimension
							R[2] = Radiation_Detector.pixel_positions_z[pixel_id] - species->particles->Position[2][particle_id];
						distance = sqrt(R[0]*R[0] + R[1]*R[1] + R[2]*R[2]);
						n[0] = R[0] / distance;
						n[1] = R[1] / distance;
						n[2] = R[2] / distance;
						detector_time = retarded_simulation_time + distance; // distance given in terms of c*t, so can be directly used there
						// get current and recent insertion indexes
						auto high = std::upper_bound(Radiation_Detector.time_array.begin(), Radiation_Detector.time_array.end(), detector_time); // index of the first element, greater than input value
						deposition_insertion_index = high - Radiation_Detector.time_array.begin() - 1;
						high = std::upper_bound(Radiation_Detector.time_array.begin(), Radiation_Detector.time_array.end(), species->particles->RecentDepositionTimes[pixel_id][particle_id]); // index of the first element, greater than input value
						previous_deposition_index = high - Radiation_Detector.time_array.begin() - 1; // should be minimum zero!
						// get the current and previous gamma factors, velocities, and then acceleration
						gf = sqrt(1 + species->particles->Momentum[0][particle_id]*species->particles->Momentum[0][particle_id]\
								+ species->particles->Momentum[1][particle_id]*species->particles->Momentum[1][particle_id]\
								+ species->particles->Momentum[2][particle_id]*species->particles->Momentum[2][particle_id]);
						gf_old = sqrt(1 + species->particles->Momentum_old[0][particle_id]*species->particles->Momentum_old[0][particle_id]\
								+ species->particles->Momentum_old[1][particle_id]*species->particles->Momentum_old[1][particle_id]\
								+ species->particles->Momentum_old[2][particle_id]*species->particles->Momentum_old[2][particle_id]);
						particle_velocity[0] = species->particles->Momentum[0][particle_id] / gf;
						particle_velocity[1] = species->particles->Momentum[1][particle_id] / gf;
						particle_velocity[2] = species->particles->Momentum[2][particle_id] / gf;
						particle_velocity_old[0] = species->particles->Momentum_old[0][particle_id] / gf_old;
						particle_velocity_old[1] = species->particles->Momentum_old[1][particle_id] / gf_old;
						particle_velocity_old[2] = species->particles->Momentum_old[2][particle_id] / gf_old;
						particle_acceleration[0] = (particle_velocity[0] - particle_velocity_old[0]) / timestep;
						particle_acceleration[1] = (particle_velocity[1] - particle_velocity_old[1]) / timestep;
						particle_acceleration[2] = (particle_velocity[2] - particle_velocity_old[2]) / timestep;
						// calculate the deposited fields by including macro-particle weight (RaDiO Eqn. 1)
						temp_holder[0] = n[0] - particle_velocity[0]; // n - velocity
						temp_holder[1] = n[1] - particle_velocity[1];
						temp_holder[2] = n[2] - particle_velocity[2];
						cross_product_holder[0] = temp_holder[1] * particle_acceleration[2] - temp_holder[2] * particle_acceleration[1]; // (n - velocity) x acceleration
						cross_product_holder[1] = temp_holder[2] * particle_acceleration[0] - temp_holder[0] * particle_acceleration[2];
						cross_product_holder[2] = temp_holder[0] * particle_acceleration[1] - temp_holder[1] * particle_acceleration[0];
						temp_holder[0] = n[1] * cross_product_holder[2] - n[2] * cross_product_holder[1]; // n x ((n - velocity) x acceleration)
						temp_holder[1] = n[2] * cross_product_holder[0] - n[0] * cross_product_holder[2];
						temp_holder[2] = n[0] * cross_product_holder[1] - n[1] * cross_product_holder[0];
						field_denominator = pow(1 - (particle_velocity[0]*n[0] + particle_velocity[1]*n[1] + particle_velocity[2]*n[2]), 3.0) * distance; // denominator section of acceleration field in RaDiO Eqn. 1
						double velocity_term_denominator = pow(gf, 2) * pow(1 - (particle_velocity[0]*n[0] + particle_velocity[1]*n[1] + particle_velocity[2]*n[2]), 3.0) * pow(distance, 2);
						// combine macro-particle weight profile and species charge properly
						weight_charge_factor = species->particles->Weight[particle_id] * params.WeightNormalizationFactor * species->particles->Charge[particle_id];
						deposited_electric_field[0] = weight_charge_factor * temp_holder[0] / field_denominator + weight_charge_factor * (n[0] - particle_velocity[0]) / velocity_term_denominator;
						deposited_electric_field[1] = weight_charge_factor * temp_holder[1] / field_denominator + weight_charge_factor * (n[1] - particle_velocity[1]) / velocity_term_denominator;
						deposited_electric_field[2] = weight_charge_factor * temp_holder[2] / field_denominator + weight_charge_factor * (n[2] - particle_velocity[2]) / velocity_term_denominator;
						deposited_magnetic_field[0] = n[1] * deposited_electric_field[2] - n[2] * deposited_electric_field[1];
						deposited_magnetic_field[1] = n[2] * deposited_electric_field[0] - n[0] * deposited_electric_field[2];
						deposited_magnetic_field[2] = n[0] * deposited_electric_field[1] - n[1] * deposited_electric_field[0];

						//printf("weight norm factor & charge: %5.14f , %3.3d  \n", species->particles->Weight[particle_id] * params.WeightNormalizationFactor, species->particles->Charge[particle_id]);
						//printf("retarded simulation time, timestep, and field detected time: %20.18f , %20.18f, %20.18f \n", retarded_simulation_time, timestep, detector_time);
						//printf("distance & normalized distance: %15.12f , %15.12f, %15.12f  &  %15.12f , %15.12f, %15.12f  \n", R[0], R[1], R[2], n[0], n[1], n[2]);
						//printf("velocity vector & acceleration vector: %15.12f , %15.12f, %15.12f  &  %15.12f , %15.12f, %15.12f  \n", particle_velocity[0], particle_velocity[1], particle_velocity[2],\
								particle_acceleration[0], particle_acceleration[1], particle_acceleration[2]);

						// algorithm decisions on nearest neighbor interpolation
						// this part does not need thread-safe operations as each patch works on specific sub-detector

						recent_deposition_time = species->particles->RecentDepositionTimes[pixel_id][particle_id];
						recent_deposited_field_x = species->particles->RecentDepositionElectricField_x[pixel_id][particle_id];
						recent_deposited_field_y = species->particles->RecentDepositionElectricField_y[pixel_id][particle_id];
						recent_deposited_field_z = species->particles->RecentDepositionElectricField_z[pixel_id][particle_id];
						recent_deposited_Bfield_x = species->particles->RecentDepositionMagneticField_x[pixel_id][particle_id];
						recent_deposited_Bfield_y = species->particles->RecentDepositionMagneticField_y[pixel_id][particle_id];
						recent_deposited_Bfield_z = species->particles->RecentDepositionMagneticField_z[pixel_id][particle_id];

						//printf("position - momentum_old - momentum : %5.5f , %5.5f, %5.5f  \n", species->particles->Position[0][particle_id], species->particles->Momentum_old[0][particle_id], species->particles->Momentum[0][particle_id] );
						//printf("MPI rank - particle_id - patch_id - recent deposition time - recent index - recent_deposited_field_y - new deposition index - deposited_field_y: %d , %d, %d, %5.5f , %d, %5.5f, %d, %5.5f  \n", smpi->getRank(), particle_id, ipatch, species->particles->RecentDepositionTimes[pixel_id][particle_id], previous_deposition_index, recent_deposited_field_y, deposition_insertion_index, deposited_electric_field[1] );


						if (detector_time > Radiation_Detector.time_array[0] && detector_time < Radiation_Detector.time_array.back()){ // deposition type will be handled according to insertion indexes
							if(deposition_insertion_index == previous_deposition_index){ // same time window
								scale_factor = (detector_time - recent_deposition_time) / Radiation_Detector.time_resolution;
								DetectorPixelFieldX[previous_deposition_index] += scale_factor * recent_deposited_field_x;
								DetectorPixelFieldY[previous_deposition_index] += scale_factor * recent_deposited_field_y;
								DetectorPixelFieldZ[previous_deposition_index] += scale_factor * recent_deposited_field_z;
								DetectorPixelBFieldX[previous_deposition_index] += scale_factor * recent_deposited_Bfield_x;
								DetectorPixelBFieldY[previous_deposition_index] += scale_factor * recent_deposited_Bfield_y;
								DetectorPixelBFieldZ[previous_deposition_index] += scale_factor * recent_deposited_Bfield_z;
							}
							else{ // nearest neighbor needed along the time cells
								if(recent_deposition_time > Radiation_Detector.time_array[0]){ // if it's the first hit to the detector, no need to deposit into current cell
									// first deposit onto the current time cell
									scale_factor = (Radiation_Detector.time_array[previous_deposition_index+1] - recent_deposition_time) / Radiation_Detector.time_resolution;
									DetectorPixelFieldX[previous_deposition_index] += scale_factor * recent_deposited_field_x;
									DetectorPixelFieldY[previous_deposition_index] += scale_factor * recent_deposited_field_y;
									DetectorPixelFieldZ[previous_deposition_index] += scale_factor * recent_deposited_field_z;
									DetectorPixelBFieldX[previous_deposition_index] += scale_factor * recent_deposited_Bfield_x;
									DetectorPixelBFieldY[previous_deposition_index] += scale_factor * recent_deposited_Bfield_y;
									DetectorPixelBFieldZ[previous_deposition_index] += scale_factor * recent_deposited_Bfield_z;
									// deposit the cells until the current one
									previous_deposition_index += 1;
									#pragma omp simd
									for(unsigned int current_index = previous_deposition_index ; current_index < deposition_insertion_index ; current_index++){
										DetectorPixelFieldX[current_index] += recent_deposited_field_x; // scale factor is 1, as it covers the full cell
										DetectorPixelFieldY[current_index] += recent_deposited_field_y;
										DetectorPixelFieldZ[current_index] += recent_deposited_field_z;
										DetectorPixelBFieldX[current_index] += recent_deposited_Bfield_x;
										DetectorPixelBFieldY[current_index] += recent_deposited_Bfield_y;
										DetectorPixelBFieldZ[current_index] += recent_deposited_Bfield_z;
									}
									// deposit the recent cell
									scale_factor = (detector_time - Radiation_Detector.time_array[deposition_insertion_index]) / Radiation_Detector.time_resolution;
									DetectorPixelFieldX[deposition_insertion_index] += scale_factor * recent_deposited_field_x;
									DetectorPixelFieldY[deposition_insertion_index] += scale_factor * recent_deposited_field_y;
									DetectorPixelFieldZ[deposition_insertion_index] += scale_factor * recent_deposited_field_z;
									DetectorPixelBFieldX[deposition_insertion_index] += scale_factor * recent_deposited_Bfield_x;
									DetectorPixelBFieldY[deposition_insertion_index] += scale_factor * recent_deposited_Bfield_y;
									DetectorPixelBFieldZ[deposition_insertion_index] += scale_factor * recent_deposited_Bfield_z;
								}
							}
							// update recent deposition time and deposited fields
							species->particles->RecentDepositionTimes[pixel_id][particle_id] = detector_time;
							species->particles->RecentDepositionElectricField_x[pixel_id][particle_id] = deposited_electric_field[0];
							species->particles->RecentDepositionElectricField_y[pixel_id][particle_id] = deposited_electric_field[1];
							species->particles->RecentDepositionElectricField_z[pixel_id][particle_id] = deposited_electric_field[2];
							species->particles->RecentDepositionMagneticField_x[pixel_id][particle_id] = deposited_magnetic_field[0];
							species->particles->RecentDepositionMagneticField_y[pixel_id][particle_id] = deposited_magnetic_field[1];
							species->particles->RecentDepositionMagneticField_z[pixel_id][particle_id] = deposited_magnetic_field[2];


							//printf("scale_factor - new deposition time - new deposited field: %5.5f , %5.5f , %5.5f \n",  scale_factor, species->particles->RecentDepositionTimes[pixel_id][particle_id], species->particles->RecentDepositionElectricField_y[pixel_id][particle_id]);
							//for(unsigned int temp_id = 0 ; temp_id < DetectorPixelFieldY.size() ; temp_id++){
							//	printf("%5.5f - ", DetectorPixelFieldY[temp_id]);
							//}
							//printf("\n");

							//printf("electric field Ex, Ey, Ez inserted on deposition index: %15.12f , %15.12f, %15.12f, %i \n",\
							//		Radiation_Detector.pixel_electric_field_x[pixel_id][deposition_insertion_index],\
							//		Radiation_Detector.pixel_electric_field_y[pixel_id][deposition_insertion_index],\
							//		Radiation_Detector.pixel_electric_field_z[pixel_id][deposition_insertion_index],\
							//		deposition_insertion_index);
						}
						else if (detector_time > Radiation_Detector.time_array.back()){ // last deposition for a given particle on the pixel, no more integration will be possible
							if(recent_deposition_time > Radiation_Detector.time_array[0]){ // if it's never hit to the detector, no need to deposit the field along the detector time array
								scale_factor = (Radiation_Detector.time_array[previous_deposition_index+1] - recent_deposition_time) / Radiation_Detector.time_resolution;
								DetectorPixelFieldX[previous_deposition_index] += scale_factor * recent_deposited_field_x;
								DetectorPixelFieldY[previous_deposition_index] += scale_factor * recent_deposited_field_y;
								DetectorPixelFieldZ[previous_deposition_index] += scale_factor * recent_deposited_field_z;
								...
								previous_deposition_index += 1;
								#pragma omp simd
								for(unsigned int current_index = previous_deposition_index ; current_index < deposition_insertion_index ; current_index++){ // go until the end of exposure
									DetectorPixelFieldX[current_index] += recent_deposited_field_x; // scale factor is 1, as it covers the full cell
									DetectorPixelFieldY[current_index] += recent_deposited_field_y;
									DetectorPixelFieldZ[current_index] += recent_deposited_field_z;
									DetectorPixelBFieldX[current_index] += recent_deposited_Bfield_x;
							...
							species->particles->RecentDepositionTimes[pixel_id][particle_id] = detector_time; // temporal integration is saturated for the given pixel
						}
						else{} // there is still time for the plane wave surface to hit the detector pixel
...
//! write deposited fields to the file at the end of simulation
void VectorPatch::dumpRadiationDetector( Params &params, SmileiMPI *smpi)
{
	//! Each MPI rank will write its own file whose filename will include its rank number, then diagnostic script will collect all of these and linearly sum the deposited electric fields.
	//! File should be opened in an "appended mode", so if it's the first write then it should be created, otherwise can be used in repeated manner by other ranks too (restart option)
	//! Due to the restart option, rank_specified filename might consist of other rank's data for another simulation run too, so the file will consist of creation times, and then
	// diagnostics script can deal with that, i.e. in which simulation run, which MPI ranks collect the written deposited fields --> with the purpose of DEBUGGING


	// firstly, linearly sum all sub-detectors (patch-based) into the main detector (MPI-based)
	for(auto const& SubDetector: ThreadBasedRadiationDetectors){
		for( unsigned int pixel_id=0; pixel_id < Radiation_Detector.pixel_number; pixel_id++ ) {
			//printf("Resulted thread-specific detector for a given MPI rank & pixel id:  %d - %d \n", smpi->getRank(), pixel_id);
			//for(unsigned int temp_id = 0 ; temp_id < SubDetector.pixel_electric_field_y[pixel_id].size() ; temp_id++){
			//	printf("%5.5f - ", SubDetector.pixel_electric_field_y[pixel_id][temp_id]);
			//}
			//printf("\n");
			std::transform( Radiation_Detector.pixel_electric_field_x[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_x[pixel_id].end( ),\
					SubDetector.pixel_electric_field_x[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_x[pixel_id].begin( ), std::plus<double>( ) );
			std::transform( Radiation_Detector.pixel_electric_field_y[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_y[pixel_id].end( ),\
					SubDetector.pixel_electric_field_y[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_y[pixel_id].begin( ), std::plus<double>( ) );
			std::transform( Radiation_Detector.pixel_electric_field_z[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_z[pixel_id].end( ),\
					SubDetector.pixel_electric_field_z[pixel_id].begin( ), Radiation_Detector.pixel_electric_field_z[pixel_id].begin( ), std::plus<double>( ) );
			std::transform( Radiation_Detector.pixel_magnetic_field_x[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_x[pixel_id].end( ),\
					SubDetector.pixel_magnetic_field_x[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_x[pixel_id].begin( ), std::plus<double>( ) );
			std::transform( Radiation_Detector.pixel_magnetic_field_y[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_y[pixel_id].end( ),\
					SubDetector.pixel_magnetic_field_y[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_y[pixel_id].begin( ), std::plus<double>( ) );
			std::transform( Radiation_Detector.pixel_magnetic_field_z[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_z[pixel_id].end( ),\
					SubDetector.pixel_magnetic_field_z[pixel_id].begin( ), Radiation_Detector.pixel_magnetic_field_z[pixel_id].begin( ), std::plus<double>( ) );

		}
	}

	//for( unsigned int pixel_id=0; pixel_id < Radiation_Detector.pixel_number; pixel_id++ ) {
	//	printf("Cumulative RANK-specific detector for a given MPI rank & pixel id:  %d - %d \n", smpi->getRank(), pixel_id);
	//	for(unsigned int temp_id = 0 ; temp_id < Radiation_Detector.pixel_electric_field_y[pixel_id].size() ; temp_id++){
	//		printf("%5.5f - ", Radiation_Detector.pixel_electric_field_y[pixel_id][temp_id]);
	//	}
	//	printf("\n");
	//}

	// dump the main detector of the given MPI rank
	if (Radiation_Detector.pixel_number > 0){
		// get the rank, the filename, and open the output file in append mode
		unsigned int MPI_rank = smpi->getRank();
		// create the RANK-based output file
		std::string filename_txt = "Radiation_Detector_" + std::to_string(MPI_rank) + ".txt";
		std::ofstream fout(filename_txt, std::ios::app);
		if(fout){
			fout << std::setprecision(15);  // numerical precision set
			// MPI-rank
			fout << std::to_string(MPI_rank) << '\n';
			// local time
			time_t rawtime;
			struct tm * timeinfo;
			char buffer[80];
			...
			strftime(buffer,sizeof(buffer),"%d-%m-%Y %H:%M:%S",timeinfo); // fixed 19 characters of date & time
			std::string time_string(buffer);
			fout << time_string << '\n';
			// detector time array size and time-grid
			fout << std::to_string(Radiation_Detector.time_array.size()) << '\n';
			for(auto const& x : Radiation_Detector.time_array)
				fout << x << ' ';
			fout << '\n';
			// detector pixel number
			fout << std::to_string(Radiation_Detector.pixel_number) << '\n';
			fout.flush();
			// deposited fields for each detector pixel
			for(unsigned int i=0; i < Radiation_Detector.pixel_positions_x.size(); i++){ // iterate through pixels
				fout << Radiation_Detector.pixel_positions_x[i] << '\n';          // pixel spatial coordinates
				fout << Radiation_Detector.pixel_positions_y[i] << '\n';
				fout << Radiation_Detector.pixel_positions_z[i] << '\n';
				for(auto const& x : Radiation_Detector.pixel_electric_field_x[i]) // Ex
					fout << x << ' ';
				fout << '\n';
				for(auto const& x : Radiation_Detector.pixel_electric_field_y[i]) // Ey
					fout << x << ' ';
				fout << '\n';
				for(auto const& x : Radiation_Detector.pixel_electric_field_z[i]) // Ez
					fout << x << ' ';
				fout << '\n';
				for(auto const& x : Radiation_Detector.pixel_magnetic_field_x[i]) // Bx
					fout << x << ' ';
				fout << '\n';
				for(auto const& x : Radiation_Detector.pixel_magnetic_field_y[i]) // By
					fout << x << ' ';
				fout << '\n';
				for(auto const& x : Radiation_Detector.pixel_magnetic_field_z[i]) // Bz
					fout << x << ' ';
				fout << '\n';
				fout.flush();
			}
			...
			ERROR( "Radiation Detector file could not be created for the MPI rank:  " << MPI_rank );


		/*
		/// Binary file in append mode implementation

		std::string filename = "Radiation_Detector_" + std::to_string(MPI_rank) + ".dat";
		std::ofstream outputFile( filename, std::ios::app | std::ios::binary);
		if(outputFile){
			//! "Rank date_time time_grid_size time_array pixel_number pixel_pos_x pixel_pos_y pixel_pos_z E_x E_y E_z\n";

			outputFile.write((const char*)&MPI_rank, sizeof(unsigned int)); // rank
			// get the local time
			time_t rawtime;
			struct tm * timeinfo;
			char buffer[80];
			...
			strftime(buffer,sizeof(buffer),"%d-%m-%Y %H:%M:%S",timeinfo); // fixed 19 characters of date & time
			std::string time_string(buffer);
			outputFile.write(time_string.c_str(), time_string.size()); // date & time
			// detector time array is common
			unsigned int time_grid_size = Radiation_Detector.time_array.size();
			outputFile.write((const char*)&time_grid_size, sizeof(unsigned int)); // time grid size
			outputFile.write((const char*)&Radiation_Detector.time_array[0], Radiation_Detector.time_array.size() * sizeof(double)); // time array
			// for each pixel, dump the deposited E-field
			outputFile.write((const char*)&Radiation_Detector.pixel_number, sizeof(unsigned int)); // pixel number
			for(unsigned int i=0; i < Radiation_Detector.pixel_positions_x.size(); i++){ // iterate through pixels
				outputFile.write((const char*)&Radiation_Detector.pixel_positions_x[i], sizeof(double)); // pixel positions
				outputFile.write((const char*)&Radiation_Detector.pixel_positions_y[i], sizeof(double));
				outputFile.write((const char*)&Radiation_Detector.pixel_positions_z[i], sizeof(double));
				outputFile.write((const char*)&Radiation_Detector.pixel_electric_field_x[i][0], Radiation_Detector.pixel_electric_field_x[i].size() * sizeof(double)); // E-fields
				outputFile.write((const char*)&Radiation_Detector.pixel_electric_field_y[i][0], Radiation_Detector.pixel_electric_field_y[i].size() * sizeof(double));
				outputFile.write((const char*)&Radiation_Detector.pixel_electric_field_z[i][0], Radiation_Detector.pixel_electric_field_z[i].size() * sizeof(double));
				outputFile.write((const char*)&Radiation_Detector.pixel_magnetic_field_x[i][0], Radiation_Detector.pixel_magnetic_field_x[i].size() * sizeof(double)); // B-fields
				outputFile.write((const char*)&Radiation_Detector.pixel_magnetic_field_y[i][0], Radiation_Detector.pixel_magnetic_field_y[i].size() * sizeof(double));
				outputFile.write((const char*)&Radiation_Detector.pixel_magnetic_field_z[i][0], Radiation_Detector.pixel_magnetic_field_z[i].size() * sizeof(double));
				outputFile.flush();
			}
			outputFile.close();
...
{
	// -#pragma omp parallel- is applied at higher level, i.e. outside the block where this call resides.
	#pragma omp for schedule(static) // NOTE: pragma for-loop scheduler should be disabled when python profile will need to be used!
    for( unsigned int ipatch=0 ; ipatch<size() ; ipatch++ ) {
    	//std::cout << "patch - pf index & pf  B_peak " << ipatch << "   " << patches_[ipatch]->EMfields->prescribedFields.front().index << "   " << patches_[ipatch]->EMfields->prescribedFields.front().B_peak << std::endl;
    ...
    #pragma omp for schedule(runtime)
        for( unsigned int ipatch=0 ; ipatch<this->size() ; ipatch++ ) {
            ...
            for( unsigned int ispec=0 ; ispec<( *this )( ipatch )->vecSpecies.size() ; ispec++ ) {
                ...
                //! to be used for calculating acceleration value within the Radiation Detector field deposition function
                spec->particles->saveMomentums();
