...
void Species::initParticles( Params &params, Patch *patch, bool with_particles, Particles * like_particles )
{
    ...
    if( params.restart || !with_particles ) {
        ...
        if( like_particles ) {
            particles->initialize( 0, *like_particles ); // particles should already know the number of Detector pixels via Species create() & clone()
        } else {
            particles->initialize( 0, params.nDim_particle, params.keep_position_old ); // particles should already know the number of Detector pixels via Species create() & clone()
        }
