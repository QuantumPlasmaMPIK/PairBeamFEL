...
public:
    static Species *create( Params &params, int ispec, Patch *patch )
    {
        ...
        //! Radiation Detector-related property for particles of the given species
        this_species->particles->number_of_field_deposition = params.pixel_positions_x.size(); // pixel number of the Detector
        this_species->particles->start_deposition_time = 0;
        if (params.detector_time_array.size() > 0){
        	this_species->particles->start_deposition_time = params.detector_time_array[0]; // measurement (exposure) start time of the Detector
        }
    ...
    static Species *clone( Species *species, Params &params, Patch *patch )
    {
        ...
        new_species->particles->number_of_field_deposition = species->particles->number_of_field_deposition;
        new_species->particles->start_deposition_time = species->particles->start_deposition_time;
    ...
    static void createVector( Params &params, Patch *patch )
    {
        ...
        //! Radiation Detector comment: all species are created via create() as above, and so all of them will have the same number of detector pixels automatically.
        // Below, corresponding species will linked to each other such as ionized electrons and BW pairs, and so no problem to init. & keep detector pixel number for those secondary species too

        // Initialize particles & operators
        for( unsigned int ispec = 0; ispec < tot_species_number; ispec++ ) {
            patch->vecSpecies[ispec]->initParticles( params, patch ); // no need to forward detector pixel number for further init. section of particles, as Species create() & clone() handle it
