...
public:
    static ElectroMagn *create( Params &params, DomainDecomposition *domain_decomposition, std::vector<Species *> &vecSpecies,  Patch *patch )
    {
        ...
        for( unsigned int n_extfield = 0; n_extfield < PyTools::nComponents( "PrescribedField" ); n_extfield++ ) {
            ...
            bool use_profile;
			double frame_gamma;
			double frame_velocity;
			double undulator_start_lab_frame;
			double undulator_end_lab_frame;
			double undulator_period;
			double B_peak;

			//! extract the prescribed field properties

            // additional attributes set
			PyTools::extract( "use_profile", extField.use_profile, "PrescribedField", n_extfield );
            PyTools::extract( "frame_gamma", extField.frame_gamma, "PrescribedField", n_extfield );
            PyTools::extract( "frame_velocity", extField.frame_velocity, "PrescribedField", n_extfield );
            PyTools::extract( "undulator_start_lab_frame", extField.undulator_start_lab_frame, "PrescribedField", n_extfield );
            PyTools::extract( "undulator_end_lab_frame", extField.undulator_end_lab_frame, "PrescribedField", n_extfield );
            PyTools::extract( "undulator_period", extField.undulator_period, "PrescribedField", n_extfield );
            PyTools::extract( "B_peak", extField.B_peak, "PrescribedField", n_extfield );
            ...
            if( first_creation ) {
                ...
                if(extField.use_profile){
                	MESSAGE(1, "Prescribed field " << fieldName << ": " << "profile provided in the namelist will be used as default");}
                else{
                	MESSAGE(1, "Prescribed field " << fieldName << ": " << "internally implemented hard-coded profile for lorentz-boosted FEL scenario will be used");}
            }
            EMfields->prescribedFields.push_back( extField );
            //std::cout << "index & B_peak " << extField.index << "   " << extField.B_peak << std::endl;
        }
    ...
    static ElectroMagn *clone( ElectroMagn *EMfields, Params &params, std::vector<Species *> &vecSpecies,  Patch *patch, unsigned int n_moved )
    {
        ...
        for( unsigned int n_extfield = 0; n_extfield < EMfields->prescribedFields.size(); n_extfield++ ) {
            ...
            // additional attributes
            newpf.use_profile   = EMfields->prescribedFields[n_extfield].use_profile;
            newpf.frame_gamma   = EMfields->prescribedFields[n_extfield].frame_gamma;
            newpf.frame_velocity   = EMfields->prescribedFields[n_extfield].frame_velocity;
            newpf.undulator_start_lab_frame   = EMfields->prescribedFields[n_extfield].undulator_start_lab_frame;
            newpf.undulator_end_lab_frame   = EMfields->prescribedFields[n_extfield].undulator_end_lab_frame;
            newpf.undulator_period   = EMfields->prescribedFields[n_extfield].undulator_period;
            newpf.B_peak   = EMfields->prescribedFields[n_extfield].B_peak;
