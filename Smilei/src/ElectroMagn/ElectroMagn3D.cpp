    ...
    isZmax( patch->isZmax() )
{
    ...
    for( unsigned int ispec=0; ispec<n_species; ispec++ ) { // end loop on ispec
        if( emFields->Jx_s[ispec] ) {
            Jx_s[ispec] = FieldFactory::create3D( dimPrim, 0, false, 
                                                  emFields->Jx_s[ispec]->name, params, 
                                                  emFields->Jx_s[ispec]->data_ != NULL );
        }
        ...
        if( emFields->rho_s[ispec] ) {
            rho_s[ispec] = FieldFactory::create3D( dimPrim, 
                                                    emFields->rho_s[ispec]->name, 
                                                    emFields->rho_s[ispec]->data_ != NULL );
        }
...
void ElectroMagn3D::initElectroMagn3DQuantities( Params &params, Patch *patch )
{
    ...
    if(use_BTIS3){
        ...
        By_mBTIS3 = FieldFactory::create3D( dimPrim, 2, false, "By_mBTIS3", params );
        Bz_mBTIS3 = FieldFactory::create3D( dimPrim, 1, false, "Bz_mBTIS3", params );
    }
...
void ElectroMagn3D::applyPrescribedField( Field *my_field,  PrescribedField *pf, Patch *patch, double time )
{
    ...
    // field calculation via for-loop
	double x; // simulation x-coordinate
	double y; // simulation y-coordinate
	double z; // simulation z-coordinate
	double x_lab, relative_x_lab; // x-coordinate in lab-frame via inverse Lorentz transformation
	double relative_z; // relative z coordinate w.r.t. z = 0 x-y plane
	double B_x, B_z; // undulator B-field in lab-frame
	double k_u = 2 * M_PI / pf->undulator_period; // wave-vector
    for( unsigned int i=0 ; i<dims[0] ; i++ ) {
        ...
        for( unsigned int j=0 ; j<dims[1] ; j++ ) {
            ...
            for( unsigned int k=0 ; k<dims[2] ; k++ ) {
				...
				y = pos[1];
				z = pos[2];
				relative_z = z - patch->global_grid_length[2] / 2;
				x_lab = pf->frame_gamma * (x + pf->frame_velocity * time); // inverse Lorentz transformation
				if (x_lab >= pf->undulator_start_lab_frame && x_lab < pf->undulator_end_lab_frame)
				{
					relative_x_lab = x_lab - pf->undulator_start_lab_frame;
					B_z = pf->B_peak * cosh(k_u * relative_z) * sin(k_u * relative_x_lab);
					B_x = pf->B_peak * sinh(k_u * relative_z) * cos(k_u * relative_x_lab);
					if(field3D->name.compare("Ey") == 0 ) // Ey field
						(*field3D)(i,j, k) += -1 * pf->frame_gamma * pf->frame_velocity * B_z;
					else if (field3D->name.compare("Bz_m") == 0 ) // Bz_m field
						(*field3D)(i,j, k) += pf->frame_gamma * B_z;
					else if (field3D->name.compare("Bx_m") == 0 ) // Bx_m field
						(*field3D)(i,j, k) += B_x;
					else
						ERROR(" Unknown field for the Lorentz-boosted framework");
				}
				else // entrance or exit region of the undulator (fringe field)
				{
					if (x_lab < pf->undulator_start_lab_frame)
						relative_x_lab = x_lab - pf->undulator_start_lab_frame;
					else
						relative_x_lab = x_lab - pf->undulator_end_lab_frame;
					B_z = pf->B_peak * cosh(k_u * relative_z) * k_u * relative_x_lab * exp(-1 * pow((k_u * relative_x_lab), 2) / 2);
					B_x = pf->B_peak * sinh(k_u * relative_z) * exp(-1 * pow((k_u * relative_x_lab), 2) / 2);
					if(field3D->name.compare("Ey") == 0 ) // Ey field
						(*field3D)(i,j,k) += -1 * pf->frame_gamma * pf->frame_velocity * B_z;
					else if (field3D->name.compare("Bz_m") == 0 ) // Bz_m field
						(*field3D)(i,j,k) += pf->frame_gamma * B_z;
