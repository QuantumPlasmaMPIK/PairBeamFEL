...
void ElectroMagn2D::applyPrescribedField( Field *my_field,  PrescribedField *pf, Patch *patch, double time )
{
	...
	double B_z; // undulator Bz-field in lab-frame
    ...
    for( unsigned int i=0 ; i<dims[0] ; i++ ) {
        ...
        for( unsigned int j=0 ; j<dims[1] ; j++ ) {
			...
			if (x_lab >= pf->undulator_start_lab_frame && x_lab < pf->undulator_end_lab_frame) // undulator field by solving the laplace equation for the magnetic potential
			{
				relative_x_lab = x_lab - pf->undulator_start_lab_frame;
				B_z = pf->B_peak * cosh(0) * sin(k_u * relative_x_lab);
				if(field2D->name.compare("Ey") == 0 ) // Ey field
					(*field2D)(i,j) += -1 * pf->frame_gamma * pf->frame_velocity * B_z;
				else if (field2D->name.compare("Bz_m") == 0 ) // Bz_m field
					(*field2D)(i,j) += pf->frame_gamma * B_z;
				else
			...
			else // entrance or exit region of the undulator (fringe field)
			{
				...
				B_z = pf->B_peak * cosh(0) * k_u * relative_x_lab * exp(-1 * pow((k_u * relative_x_lab), 2) / 2);
				if(field2D->name.compare("Ey") == 0 ) // Ey field
				{
					(*field2D)(i,j) += -1 * pf->frame_gamma * pf->frame_velocity * B_z;
				}
