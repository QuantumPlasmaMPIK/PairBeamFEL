...
void ElectroMagn::applyPrescribedFields( Patch *patch, double time )
{
    for( vector<PrescribedField>::iterator pf=prescribedFields.begin(); pf!=prescribedFields.end(); pf++ ) {
        if( pf->index < allFields.size() ) {
            ...
            if (pf->use_profile) // user-defined profile will be called, so python interface will be used.
            	applyPrescribedField( allFields[pf->index], pf->profile, patch, time );
			else // internal hard-coded prescribed field calculation will be used, currently supports Lorentz-boosted FEl scenario
				applyPrescribedField( allFields[pf->index], &(*pf), patch, time );
        }
    }
}
