    ...
    tracked( false )
{
    ...
    Momentum.resize( 0 );
    Momentum_old.resize( 0 );
    RecentDepositionTimes.resize(0);
    RecentDepositionElectricField_x.resize(0);
    RecentDepositionElectricField_y.resize(0);
    RecentDepositionElectricField_z.resize(0);
    RecentDepositionMagneticField_x.resize(0);
    RecentDepositionMagneticField_y.resize(0);
    RecentDepositionMagneticField_z.resize(0);
    cell_keys.resize( 0 );
    is_test = false;
    ...
    // Radiation Detector-related property
    number_of_field_deposition = 1; // as a default, there's considered to be a single-point detector exists
    start_deposition_time = 0 ; //there will be a pre-condition within the field deposition function, to check zero value and set it correctly to the Detector start time.
...
void Particles::initialize( unsigned int nParticles, unsigned int nDim, bool keep_position_old)
{
    ...
    if( double_prop_.empty() ) { // do this just once
        ...
        for( unsigned int i = 0; i < 3; i++ ) {
			double_prop_.push_back( &( Momentum_old[i] ) );
		}


        // Radiation Detector-related property
        for( unsigned int i = 0; i < number_of_field_deposition; i++ ) {
			double_prop_.push_back( &( RecentDepositionTimes[i] ) );
			double_prop_.push_back( &( RecentDepositionElectricField_x[i] ) );
			double_prop_.push_back( &( RecentDepositionElectricField_y[i] ) );
			double_prop_.push_back( &( RecentDepositionElectricField_z[i] ) );
			double_prop_.push_back( &( RecentDepositionMagneticField_x[i] ) );
			double_prop_.push_back( &( RecentDepositionMagneticField_y[i] ) );
			double_prop_.push_back( &( RecentDepositionMagneticField_z[i] ) );
		}
...
void Particles::initialize( unsigned int nParticles, Particles &part )
{
    ...
    // Radiation Detector-related property
    number_of_field_deposition = part.number_of_field_deposition;
    start_deposition_time = part.start_deposition_time;
                         ...
                         bool         keep_position_old )
{
    ...
    Momentum_old.resize( 3 );
	for( unsigned int i = 0; i < 3; i++ ) {
		Momentum_old[i].reserve( reserved_particles );
	}

    Weight.reserve( reserved_particles );
    Charge.reserve( reserved_particles );

    // Radiation Detector-related properties
    RecentDepositionTimes.resize(number_of_field_deposition);
    RecentDepositionElectricField_x.resize(number_of_field_deposition);
    RecentDepositionElectricField_y.resize(number_of_field_deposition);
    RecentDepositionElectricField_z.resize(number_of_field_deposition);
    RecentDepositionMagneticField_x.resize(number_of_field_deposition);
	RecentDepositionMagneticField_y.resize(number_of_field_deposition);
	RecentDepositionMagneticField_z.resize(number_of_field_deposition);
    for( unsigned int i = 0; i < number_of_field_deposition; i++ ) {
    	RecentDepositionTimes[i].reserve( reserved_particles );
    	RecentDepositionElectricField_x[i].reserve( reserved_particles );
    	RecentDepositionElectricField_y[i].reserve( reserved_particles );
    	RecentDepositionElectricField_z[i].reserve( reserved_particles );
    	RecentDepositionMagneticField_x[i].reserve( reserved_particles );
		RecentDepositionMagneticField_y[i].reserve( reserved_particles );
		RecentDepositionMagneticField_z[i].reserve( reserved_particles );
	}
                        ...
                        bool         keep_position_old )
{
    ...
    Momentum_old.resize( 3 );
	for( unsigned int i = 0; i < 3; i++ ) {
		Momentum_old[i].resize( nParticles, 0. );
	}
	...
	RecentDepositionMagneticField_z.resize(number_of_field_deposition);
    for( unsigned int i = 0; i < number_of_field_deposition; i++ ) {
		RecentDepositionTimes[i].resize( nParticles, start_deposition_time ); // even if it's not set correctly yet, it can be fixed in the deposition field calculation via pre-check
		RecentDepositionElectricField_x[i].resize( nParticles, 0. );          // already correct initialization for the field values
		RecentDepositionElectricField_y[i].resize( nParticles, 0. );
		RecentDepositionElectricField_z[i].resize( nParticles, 0. );
		RecentDepositionMagneticField_x[i].resize( nParticles, 0. );          // already correct initialization for the field values
		RecentDepositionMagneticField_y[i].resize( nParticles, 0. );
		RecentDepositionMagneticField_z[i].resize( nParticles, 0. );
	}
...
void Particles::makeParticleAt( Particles &source_particles, unsigned int ipart, double w, short q, double px, double py, double pz )
{
    ...
    Momentum_old[0].push_back( px );
	Momentum_old[1].push_back( py );
	Momentum_old[2].push_back( pz );
    ...
    // Radiation Detector-related property
    for( unsigned int i=0 ; i < number_of_field_deposition ; i++ ) {
		RecentDepositionTimes[i].push_back( source_particles.RecentDepositionTimes[i][ipart] );
		RecentDepositionElectricField_x[i].push_back( source_particles.RecentDepositionElectricField_x[i][ipart] );
		RecentDepositionElectricField_y[i].push_back( source_particles.RecentDepositionElectricField_y[i][ipart] );
		RecentDepositionElectricField_z[i].push_back( source_particles.RecentDepositionElectricField_z[i][ipart] );
		RecentDepositionMagneticField_x[i].push_back( source_particles.RecentDepositionMagneticField_x[i][ipart] );
		RecentDepositionMagneticField_y[i].push_back( source_particles.RecentDepositionMagneticField_y[i][ipart] );
		RecentDepositionMagneticField_z[i].push_back( source_particles.RecentDepositionMagneticField_z[i][ipart] );
	}
...
void Particles::print( unsigned int iPart )
{
    ...
    // Radiation Detector-related property
    cout << RecentDepositionTimes[0][iPart] << endl; // print only the first pixel as default
...
ostream &operator << ( ostream &out, const Particles &particles )
{
    for( unsigned int iPart=0; iPart<particles.Weight.size(); iPart++ ) {
        ...
        // Radiation Detector-related property
        out << particles.RecentDepositionTimes[0][iPart] << endl; // print only the first pixel as default
...
void Particles::saveMomentums() {
    unsigned int ndim = 3, npart = size(); // Momentum vector already has 3 dimensions regardless of position dimension
    double *p[3], *pold[3];
    for( unsigned int i = 0 ; i<ndim ; i++ ) {
        p[i] =  &( Momentum[i][0] );
        pold[i] =  &( Momentum_old[i][0] );
    }
