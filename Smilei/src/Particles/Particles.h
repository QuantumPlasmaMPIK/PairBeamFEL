...
public:
    ...
    //! Method to keep the momentums for the next timesteps
	void saveMomentums();
    ...
    //! Return the pointer toward the old Momentum[idim] vector
	virtual double* getPtrMomentumOld( int idim ) {
		return ((std::size_t)idim < Momentum_old.size()) ? Momentum_old[idim].data() : nullptr;
	};
    ...
    //! array of particle former (old) momenta
	std::vector< std::vector<double> > Momentum_old;

    //! array of particle weights: equivalent to a density normalized to the number of macro-particles per cell
    std::vector<double> Weight;

    //! array of Deposited Radiation properties for the RaDiO Algorithm, array sizes will be adapted run-time and set to number of pixels X number of particles
    unsigned int number_of_field_deposition; // this should be equal to detector pixel number at the end.
    double start_deposition_time; // this should be set to the first element of detector exposure time grid, which corresponds to the start time of measurement
    std::vector<std::vector<double>> RecentDepositionTimes; // pixel X particle
    std::vector<std::vector<double>> RecentDepositionElectricField_x;
    std::vector<std::vector<double>> RecentDepositionElectricField_y;
    std::vector<std::vector<double>> RecentDepositionElectricField_z;
    std::vector<std::vector<double>> RecentDepositionMagneticField_x;
	std::vector<std::vector<double>> RecentDepositionMagneticField_y;
	std::vector<std::vector<double>> RecentDepositionMagneticField_z;
