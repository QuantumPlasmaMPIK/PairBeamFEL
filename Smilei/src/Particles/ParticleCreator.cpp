                             ...
                             unsigned int itime)
{
    ...
    if( species_->position_initialization_array_ == NULL
     && species_->file_position_npart_ == 0 ) {
        // Increase array size
        particles_->initialize( n_existing_particles + n_new_particles, species_->nDim_particle, params.keep_position_old ); // associated particles should already know the number of Detector pixels via Species create() & clone()
