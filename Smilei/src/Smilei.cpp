...
int main( int argc, char *argv[] )
{
    ...
    // No restart, we initialize a new simulation
        ...
        // TODO: no need to include prescribed fields initially
    ...
    //! Initialize the radiation detector (RaDiO)!
    TITLE( "Initializing the radiation detector (RaDiO)..." );
    params.N_r = params.eps0 * pow(params.reference_angular_frequency_SI, 2) * params.electron_mass / pow(params.electron_charge, 2);
    params.WeightNormalizationFactor = pow(params.c_vacuum_ / params.reference_angular_frequency_SI, 3) * params.N_r;
    //std::cout << params.N_r << " , " << params.WeightNormalizationFactor << std::endl;
    vecPatches.initRadiationDetector(params, &smpi);
    ...
    */
    ...
    while( ( itime <= params.n_time ) && ( !checkpoint.exit_asap ) ) {
        ...
        #pragma omp parallel shared (time_dual,smpi,params, vecPatches, region, simWindow, checkpoint, itime)
        {
            ...
            //! Deposit fields over the Radiation Detector
			vecPatches.depositFields(params, &smpi, time_prim, params.timestep);
			// no need to put an external OMP barrier statement here to be on the safe side, as an implicit barrier exists at the end of the for loop


			// NOTE: There should be a single place to call prescribed fields regardless, so whether multiple decomposition is enabled or not.
			// NOTE: if you'd like to see the prescribed fields embedded into the mesh fields in the Field diagnostics frames, move the diagnostics block up to here, call it just after there!
			//if( !params.multiple_decomposition ) {
			if( time_dual > params.time_fields_frozen ) {
				// Standard fields operations (maxwell + comms + boundary conditions) are already completed within the recent timestep, so now external field can be applied before the Pusher
				// apply prescribed fields can be considered if requested
				if( vecPatches(0)->EMfields->prescribedFields.size() ) {
					//#pragma omp single // NOTE: pragma single & barrier wrapper should be enabled when python profile will need to be used!
					vecPatches.applyPrescribedFields( time_prim );
					//#pragma omp barrier
				}
			}
        ...
        if (!params.multiple_decomposition) {
            if( time_dual > params.time_fields_frozen ) {
                #pragma omp parallel shared (time_dual,smpi,params, vecPatches, region, simWindow, checkpoint, itime)
                {
					...
					//#pragma omp barrier  // --> no need an explicit barrier as worksharing construct has implicit one at the end
            ...
            if( time_dual > params.time_fields_frozen ) {
                ...
                // NOTE: this part is removed for the case of multiple decomposition, as there should be a single place to call prescribed fields regardless.
                ...
                // TODO: should not it call resetPrescribedFields over its VectorPatch before solving Maxwell?
...
// #endif
    ...
    //! Dump the Radiation Detector fields (RANK-based dump)
    vecPatches.dumpRadiationDetector(params, &smpi);
