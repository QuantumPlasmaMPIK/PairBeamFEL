    ...
    namelist( "" )
{
    ...
    // RaDiO Detector parameters
    int nDetector = PyTools::nComponents( "RadiationDetector" );
    if (nDetector > 0){
    	PyTools::extractV( "pixel_positions_x", pixel_positions_x, "RadiationDetector" );
		PyTools::extractV( "pixel_positions_y", pixel_positions_y, "RadiationDetector" );
		PyTools::extractV( "pixel_positions_z", pixel_positions_z, "RadiationDetector" );
		PyTools::extractV( "detector_time_array", detector_time_array, "RadiationDetector" );
		if (!(pixel_positions_x.size() == pixel_positions_y.size() && pixel_positions_y.size() == pixel_positions_z.size())){
			ERROR_NAMELIST( "Pixel position arrays (x, y, z) should have equal sizes",  LINK_NAMELIST + std::string("#radiation-detector"));
		}
    }
